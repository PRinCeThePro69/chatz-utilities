...
module.exports = {
    ...
    let logch = guilld.channels.cache.get('931558609194737786')
      const time = interaction.options.getInteger('time')
		 ...
		 if (time > 21600) return interaction.reply({content: "Error: maximum time is 21600 seconds (6 hours)", ephemeral: true});
     if (time < 0) return interaction.reply({content: "Error: minimum time is 0 seconds.", ephemeral: true});
      if (time === 0){
interaction.reply({content: "Slowmode has been disabled."})
      }  else {
      interaction.reply({content: `Slowmode has been set to \`${time}\` seconds.`})
      }
